...
class Day3CrossedWires {
	...
	int calculateClosestDistance(String input) {
		def lines = input.split('\n')
		String[] wire1 = lines[0].split(',')
		String[] wire2 = lines[1].split(',')
		byte[][] matrix = new byte[SIZE][SIZE]
		printWire(matrix, wire1, (byte)1)
		printWire(matrix, wire2, (byte)2)
		Set<Integer> distances = getAllIntersectionDistances(matrix)
		return distances.min()
	}

	int[] getAllIntersectionDistances(byte[][] matrix) {
		Set<Integer> distances = []
		for (int x = 0; x < SIZE; x++) {
			for (int y = 0; y < SIZE; y++) {
				if (matrix[x][y] == (byte)3) {
					distances << Math.abs(HALF_SIZE - x) + Math.abs(HALF_SIZE - y)
				}
			}
		}
		distances
	...
	private void printWire(byte[][] matrix, String[] wire, byte wireCode) {
		int x = HALF_SIZE
		int y = HALF_SIZE
		wire.each {
			int count = Integer.parseInt(it.substring(1))
			String direction = it[0]
			(1..count).each {
				switch (direction) {
					case 'L':
						matrix[--x][y] |= wireCode
