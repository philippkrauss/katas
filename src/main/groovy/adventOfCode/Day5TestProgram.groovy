...
class Day5TestProgram {
	...
	List<Integer> calculateOutput(List<Integer> program) {
		List<Integer> output = []
		int opCounter = 0
		OpCode opCode = new OpCode(program[opCounter])
		while (opCode.code != 99) {
			if (opCode.code == 1) {
				int pos1 = program[opCounter + 1]
				int pos1Value = opCode.mode1 ? pos1 : program[pos1]
				int pos2 = program[opCounter + 2]
				int pos2Value = opCode.mode2 ? pos2 : program[pos2]
